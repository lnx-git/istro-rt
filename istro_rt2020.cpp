...
example:
istro_rt2020 -cb /dev/ttyUSB0 -gps /dev/ttyUSB1 -lidar /dev/ttyUSB2 -ahrs /dev/ttyACM0 -nowait
***********************************************************************************************************************/
...
int initVision(void)
{
    ...
    LOGM_INFO(loggerIstro, "initVision", "msg=\"vision.init()\"");
    vision.init(sampleOnRoad, sampleOffRoad);

#ifdef ISTRO_VISION_DEPTH
    LOGM_INFO(loggerIstro, "initVision", "msg=\"visiond_vsn.init()\"");
...
{
    if (visionn.init() < 0) {
        LOGM_ERROR(loggerIstro, "initVisioNN", "error: -1!");
        return -1;
    }

    if (visionn.send_hello() < 0) {
        LOGM_ERROR(loggerIstro, "initVisioNN", "error: -2!");
        return -2;
    }

    if (visionn.recv_hello() < 0) {
        LOGM_ERROR(loggerIstro, "initVisioNN", "error: -3!");
        return -3;
    }
...
LOGM_DEBUG(loggerIstro, "gps_thread", "msg=\"navp_azimuth hotfix - fixme...\""
        << ", navp_azimuth=" << ioff(gps_navp_azimuth, 2) 
        << ", navp_azimuth_old=" << ioff(nazimuth, 2) );
}
#else
...
*/
        ...
        if (gps_writeData(gps_time, gps_fix, gps_latitude, gps_longitude, gps_latitude_raw, gps_longitude_raw, 
                ...
                gps_ref, gps_x, gps_y, gps_ref_latitude, gps_ref_longitude, nmap_flags, coneseek_stop, total_dist) < 0) {
            result = -4;
            break;
        }
...
void *capture_camera_thread(void *parg)
{
    ...
    while ((!thread_testcancel()) && conf.useCamera) {
        ...
        if (conf.useCamera) {
            ...
            t = timeBegin();
            if (camera.getFrameDepth(pdata->camera_depth) >= 0) {
...
void *vision_thread(void *parg)
{
    ...
    Mat camera_depth_pred0;
    Mat visiond_markers;
    Mat visiond_markersIM;
    Mat visiond_epweight;
    Mat visiond_elweight;
#endif
...
#ifdef ISTRO_VISIONN
        double t2 = timeBegin();
        if (visionn.send_imgpr(pdata->camera_img) < 0) {
            LOGM_ERROR(loggerIstro, "vision_thread", "msg=\"error: visionn.send_imgpr() failed!\"");
        }
        timeEnd("istro::vision_thread.visionn_send", t2);
#endif

#ifdef ISTRO_VISION_DEPTH
        double t3 = timeBegin();
        visiond.process(pdata->camera_depth, camera_depth_pred0, pdata->camera_depth_pred);
        visiond_vsn.eval(pdata->camera_depth_pred, visiond_markers, visiond_markersIM, visiond_epweight, visiond_elweight, visiond_dmap);
        timeEnd("istro::vision_thread.visiond_process", t3);
#endif

#ifdef ISTRO_VISIONN
        t2 = timeBegin();
        if (!pdata->camera_img_pred.empty()) {
            pdata->camera_img_pred.release();
        }
        if (visionn.recv_imgpr(pdata->camera_img_pred) < 0) {
            LOGM_ERROR(loggerIstro, "vision_thread", "msg=\"error: visionn.recv_imgpr() failed!\"");
        }
        timeEnd("istro::vision_thread.visionn_recv", t2);
        vision.eval(pdata->camera_img_pred, pdata->vision_markers, pdata->vision_markersIM, pdata->vision_epweight, pdata->vision_elweight, pdata->vision_dmap);
#else
        vision.eval(pdata->camera_img, pdata->vision_markers, pdata->vision_markersIM, pdata->vision_epweight, pdata->vision_elweight, pdata->vision_dmap);
#endif

#ifdef ISTRO_VISION_DEPTH
        //int fixme_vision_dmap_print;
        //pdata->vision_dmap.print("vision_dmap0");
        //visiond_dmap.print("visiond_dmap");

        /* vsetky vysledky VISION_DEPTH aplikujeme do "vision" vysledkov - vznikne jeden vysledny obrazok */
        double t4 = timeBegin();
        pdata->vision_dmap.apply(visiond_dmap);
        vision.applyEPWeight(pdata->vision_epweight, pdata->vision_elweight, visiond_epweight, visiond_elweight);
        vision.applyMarkers(pdata->vision_markers, visiond_markers);
        timeEnd("istro::vision_thread.visiond_apply", t4);
#endif        
...
const int CALIB_TIMEOUT1        = 12000000;
const int CALIB_TIMEOUT2        =   120000;
...
const double CALIB_PROCESS_ANGLE_MIN   =  85.0;
const double CALIB_PROCESS_ANGLE_MAX   =  95.0;
...
int calib_reset(calib_t &calib, int idx, const string &log_reason, const string &log_str)
{
    // reset calibration time, if obstacle appeared during calibration
    if ((!calib.ok) && (calib.time >= 0)) {
        calib.time = -1;
        
        LOGM_INFO(loggerIstro, "calib_reset" + to_string(idx), "msg=\"calibration interrupted!\", reason=\"" << log_reason 
...
int calib_process(calib_t &calib, int idx, double nav_angle, int process_angle_min, int process_angle_max,
        double gps_course, double yaw, double gps_speed, double navp_dist, double navp_maxdist, double ctrlb_ircv500)
{
    ...
    if (calib.ok) {
        ...
        if (timeDelta(calib.time) > (idx==1?CALIB_TIMEOUT1:CALIB_TIMEOUT2)) {
            calib.ok = 0;
            calib.time = -1;
            calib.navp_ok = -1;
            calib.nav_angle_ok = -1;
            LOGM_INFO(loggerIstro, "calib_process" + to_string(idx), "msg=\"calibration needed (timeout)!\", calib.ok=" << calib.ok);
        }
    }
    ...
    if (nav_angle_ok != calib.nav_angle_ok) {
        LOGM_DEBUG(loggerIstro, "calib_process" + to_string(idx), "msg=\"calibration - nav_angle_ok changed...\", calib.ok=" << calib.ok
    ...
    if ((navp_dist >= 0) && (navp_maxdist >= 0)) {
        ...
        if (navp_ok != calib.navp_ok) {
            if ((!navp_ok) && (!calib.ok) && (calib.time >= 0)) {
                ...
                calib_reset(calib, idx, "close to navigation point", ss.str());
            } else {
                LOGM_DEBUG(loggerIstro, "calib_process" + to_string(idx), "msg=\"calibration - navp_ok changed...\", calib.ok=" << calib.ok
    ...
    if (calib.time < 0) {
        ...
        LOGM_INFO(loggerIstro, "calib_process" + to_string(idx), "msg=\"calibration start!\", calib.ok=" << calib.ok 
            << ", calib.navp_ok=" << calib.navp_ok << ", calib.nav_angle_ok=" << calib.nav_angle_ok);
    } 

    // log all data checked during calibration
    LOGM_TRACE(loggerIstro, "calib_process" + to_string(idx), "msg=\"calibration in progress...\", calib.ok=" << calib.ok 
    ...
    if ((calib.time >= 0) && (timeDelta(calib.time) >= CALIB_TIME1_SPEED_CHK)) {
        ...
        if (gps_speed < CALIB_GPS_SPEED_MIN_VALUE) {
            ...
            calib_reset(calib, idx, "low gps speed", ss.str());
        }
        if (ctrlb_ircv500 < CALIB_IRCV500_MIN_VALUE) {
            stringstream ss;
            ss << ", ctrlb_ircv500=" << ioff(ctrlb_ircv500, 2);
            calib_reset(calib, idx, "low ircv500", ss.str());
        }
    }
    ...
    if ((calib.time >= 0) && (timeDelta(calib.time) >= CALIB_TIME2_ANGLE_CHK)) {
        ...
        if ((calib.gps_course_max - calib.gps_course_min) > CALIB_GPS_COURSE_MAX_DIFF) {
            ...
            calib_reset(calib, idx, "gps_course difference", ss.str());
        }
        if ((calib.yaw_max - calib.yaw_min) > CALIB_YAW_MAX_DIFF) {
            ...
            calib_reset(calib, idx, "yaw difference", ss.str());
        }
    }
    ...
    if ((calib.time >= 0) && (timeDelta(calib.time) >= 7000)) {
        ...
        calib.yaw = yaw;
        calib.time = -1;
        LOGM_INFO(loggerIstro, "calib_process" + to_string(idx), "msg=\"calibration finished!\", calib.ok=" << calib.ok
...
const int WRONGWAY_TIME1_STOP   =  3000;
const int WRONGWAY_TIME2_BACK   =  3250;
const int WRONGWAY_TIME3_STOP   =  3500;
const int WRONGWAY_TIME4_BACK   =  6000;  // 8000;
const int WRONGWAY_TIME5_STOP   =  9000;  // 11000;

const int WRONGWAY_TIME_DISABLED =  1000;
...
const double WRONGWAY_IRCV500_MIN_VALUE   =   5.0;  // najnizsia rychlost, pri ktorej uz zvacsujeme wrongway timeout (stojime)
...
int wrongway_check(wrongway_t &wrongway, double yaw, double angle, double navp_dist, double navp_maxdist, double ctrlb_ircv500)
// (angle > -180) && (angle <= 180)
{
    if (!wrongway.ok) {
        return 0;
    ...
    if (ctrlb_ircv500 < WRONGWAY_IRCV500_MIN_VALUE) {
        if (wrongway.ircv_low_time >= 0) {
            // napocitavaj celkovy cas, ako dlho sme stali
            wrongway.ircv_low_dt += timeDelta(wrongway.ircv_low_time);
        }
        wrongway.ircv_low_time = timeBegin();
        LOGM_INFO(loggerIstro, "wrongway_check", "msg=\"ircv speed LOW!\", wrongway.ircv_low_dt=" << ioff(wrongway.ircv_low_dt, 3));
    } else {
        if (wrongway.ircv_low_time >= 0) {
            wrongway.ircv_low_time = -1;
            LOGM_INFO(loggerIstro, "wrongway_check", "msg=\"ircv speed OK!\", wrongway.ircv_low_dt=" << ioff(wrongway.ircv_low_dt, 3));
        }
    }
    ...
    if (timeDelta(wrongway.time) >= WRONGWAY_TIMEOUT + wrongway.ircv_low_dt) {
        wrongway.ok = 0;
        wrongway.time = -1;
        wrongway.yaw0 = yaw0;
...
int wrongway_process(wrongway_t &wrongway, double yaw, int &process_angle, int &process_velocity, int &process_state)
{
...
#ifdef ISTRO_WRONGWAY_DISABLE
    if (wrongway.time < 0) {
        wrongway.time = timeBegin();
        LOGM_INFO(loggerIstro, "wrongway_process", "msg=\"wrongway start (disabled)!\", wrongway.ok=" << wrongway.ok);
    }
    // after 1sec automatically enable wrongway-check
    if (timeDelta(wrongway.time) >= WRONGWAY_TIME_DISABLED) {
        wrongway.ok = 1;
        wrongway.time = -1;
        LOGM_INFO(loggerIstro, "wrongway_process", "msg=\"wrongway finished (disabled)!\", wrongway.ok=" << wrongway.ok);
    }
    return 0;
#else
    ...
    if (timeDelta(wrongway.time) < WRONGWAY_TIME2_BACK) {
        ...
        process_state = PROCESS_STATE_WRONGWAY;
        LOGM_TRACE(loggerIstro, "wrongway_process", "msg=\"wrongway in progress (BACK2)...!\", wrongway.ok=" << wrongway.ok
    ...
    if (timeDelta(wrongway.time) < WRONGWAY_TIME3_STOP) {
        ...
        process_velocity = VEL_ZERO;
        process_state = PROCESS_STATE_WRONGWAY;
        LOGM_TRACE(loggerIstro, "wrongway_process", "msg=\"wrongway in progress (STOP3)...!\", wrongway.ok=" << wrongway.ok
            << ", wrongway.yaw0=" << ioff(wrongway.yaw0, 2) << ", yaw=" << ioff(yaw, 2) 
            << ", process_angle=" << process_angle << ", process_velocity=" << process_velocity);
    } else
    if (timeDelta(wrongway.time) < WRONGWAY_TIME4_BACK) {
        ...
        process_velocity = VEL_ZERO + conf.velocityBack;
        process_state = PROCESS_STATE_WRONGWAY;
        LOGM_TRACE(loggerIstro, "wrongway_process", "msg=\"wrongway in progress (BACK4)...!\", wrongway.ok=" << wrongway.ok
            << ", wrongway.yaw0=" << ioff(wrongway.yaw0, 2) << ", yaw=" << ioff(yaw, 2) << ", angle=" << ioff(angle, 2) 
            << ", process_angle=" << process_angle << ", process_velocity=" << process_velocity);
    } else
    if (timeDelta(wrongway.time) < WRONGWAY_TIME5_STOP) {
        ...
        process_velocity = VEL_ZERO;
        process_state = PROCESS_STATE_WRONGWAY;
        LOGM_TRACE(loggerIstro, "wrongway_process", "msg=\"wrongway in progress (STOP5)...!\", wrongway.ok=" << wrongway.ok
            << ", wrongway.yaw0=" << ioff(wrongway.yaw0, 2) << ", yaw=" << ioff(yaw, 2) 
            << ", process_angle=" << process_angle << ", process_velocity=" << process_velocity);
    } else {
        wrongway.ok = 1;
...
const int    VISION_LAG_PERIOD = 4*600;  // if no data are comming from vision, force process_stop
...
void *process_thread(void *parg)
{
    ...
    int lidar_stop = -1;
    int vision_stop = -1;
    double vision_lastt = -1;  // last time when the vision correctly processed an image    
    
    calib_t calib;
    calib_init(calib);
#ifdef ISTRO_CALIB2
    calib_t calib2;            // iba na priebezne kontrolovanie pocas kalibracnych hodnot pocas jazdy - nepouziva sa pre navigaciu
    calib_init(calib2);
#endif    
    ...
    while (!thread_testcancel() && (conf.useCamera || conf.useLidar)) {
        ...
        if (conf.useCamera) {
            ...
            // to avoid VISION_LAG_PERIOD if camera is not used
            vision_lastt = timeBegin();
        }
        ...
        if (conf.useLidar) {
            ...
            if (pdata != NULL) {
                ...
                // clear camera_img to inform save_thread, that only lidar data should be written to disk
                if (!pdata->camera_img.empty()) {
                    pdata->camera_img.release();
                }
        ...
        vision_stop = (vision_lastt < 0) || (timeDelta(vision_lastt) >= VISION_LAG_PERIOD);

        /* process_stop */
        int process_stop_old = process_stop;        
        process_stop = (vision_stop > 0) || (lidar_stop > 0);
        pdata->process_stop = process_stop;
        
        if (process_stop != process_stop_old) {
            LOGM_INFO(loggerIstro, "process_thread", "msg=\"process_stop changed!\", process_stop=" << pdata->process_stop <<
                ", vision_stop=" << vision_stop << ", lidar_stop=" << lidar_stop << ", process_stop_old=" << process_stop_old);
        }
...
*/
        ...
        if ((yawn < ANGLE_OK) && (nav_angle < ANGLE_OK)) {
            // (angle > -180) && (angle <= 180)
            wrongway_check(wrongway, yawn, nav_angle, pdata->gps_navp_dist, pdata->gps_navp_maxdist, pdata->ctrlb_ircv500);
        }
...
//pdata->process_angle_max = 180;
        ...
        if (detectobst.noobstacle <= 0) {
            ...
            calib_res = calib_reset(calib, 1, "obstacle", ss.str());
#ifdef ISTRO_CALIB2
            calib_reset(calib2, 2, "obstacle", ss.str());
#endif
        } else 
        if (pdata->process_angle < (int)ANGLE_OK) {
            calib_res = calib_reset(calib, 1, "loadarea", "");
#ifdef ISTRO_CALIB2
            calib_reset(calib2, 2, "loadarea", "");
#endif
        } else {
            calib_res = calib_process(calib, 1, nav_angle, pdata->process_angle_min, pdata->process_angle_max,
                            pdata->gps_course, yaws, pdata->gps_speed, pdata->gps_navp_dist, pdata->gps_navp_maxdist, pdata->ctrlb_ircv500);
#ifdef ISTRO_CALIB2
            calib_process(calib2, 2, nav_angle, pdata->process_angle_min, pdata->process_angle_max,
                            pdata->gps_course, yaws, pdata->gps_speed, pdata->gps_navp_dist, pdata->gps_navp_maxdist, pdata->ctrlb_ircv500);                         
#endif                            
        }
...
#ifdef ISTRO_CALIB2
        /* calibration check while driving */
        if (pdata->process_angle >= (int)ANGLE_OK) {
            calib_reset(calib2, 2, "process_noangle", "");
        } else {
            if ((pdata->process_angle < CALIB_PROCESS_ANGLE_MIN) || (pdata->process_angle > CALIB_PROCESS_ANGLE_MAX)) {
                calib_reset(calib2, 2, "process_angle_check", "");
            }
        }
#endif
        ...
        // skusime zakomentovat aktualizovanie process_x a process_y na zaklade GPS - ci budu chyby/skoky v lokalnej mape mensie
        int fixme_no_process_gps_xy;
/*
...
//  long tick_number = getTickCount();
    ...
    sprintf(filename, "%srt2020_%07u_%07u_%s.%s", outputDir.c_str(), (unsigned int)save_rand, (unsigned int)(image_number<0?999999:image_number), str.c_str(), ext.c_str());
    imwrite(filename, img);
    sprintf(filename, "rt2020_%07u_%07u_%s.%s", (unsigned int)save_rand, (unsigned int)(image_number<0?999999:image_number), str.c_str(), ext.c_str());
    ...
    while (!thread_testcancel()) {
        ...
        if (!conf.useNosave) { 
            ...
            if ((conf.useCamera) && (!pdata->camera_img.empty()) && !save_camera) {
                ...
                vision.drawOutput(pdata->camera_img, vision_img, pdata->vision_markers, pdata->vision_epweight, pdata->vision_elweight, pdata->vision_angle_min, pdata->vision_angle_max);
                saveImage(pdata->image_number, "vision", "jpg", vision_img);

                if (!pdata->camera_img_pred.empty()) {
                    saveImage(pdata->image_number, "nnpred", "png", pdata->camera_img_pred);
                }

                if (!pdata->camera_depth.empty()) {
                    camera.drawDepthFrame(pdata->camera_depth, camera_depth_img, pdata->image_number);
                    saveImage(pdata->image_number, "cdepth", "jpg", camera_depth_img);
                }
            }
